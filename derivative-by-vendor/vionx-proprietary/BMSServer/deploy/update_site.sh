...
case "$2" in
  ...
  "bms-ui")
    ...
    ansible-playbook -v bms-web-setup.yml -K --limit $1
    ;;
  "devapp")
    echo Updating DeviceService, VECProxy And Config Files
    ansible-playbook vecpackage-devapp.yml --limit $1 -K
    ssh ${REMOTE_WEB_USER}@$1 ansible-playbook /opt/deploy/devapp-update.yml --limit Sys-Units
    ;;
  "devapp-dbg")
    echo Updating DeviceService And VECProxy Binary Only
    ansible-playbook vecpackage-devapp.yml --limit $1 -K
    ssh ${REMOTE_WEB_USER}@$1 ansible-playbook /opt/deploy/devapp-updatedbg.yml --limit Sys-Units
    ;;
  "devui")
    echo Updating Device Web UI
    ansible-playbook vecpackage-ui.yml --limit $1
    ssh ${REMOTE_WEB_USER}@$1 ansible-playbook /opt/deploy/devui-update.yml --limit Sys-Units
    ;;
  "devconfig")
    echo Configuration
    ansible-playbook vecpackage-unitconfig.yml --limit $1
    ssh ${REMOTE_WEB_USER}@$1 ansible-playbook /opt/deploy/dev-unitconfig.yml --limit Sys-Units
    ;;
  "devimage")
    echo Updating VEC image files
    ansible-playbook vecpackage-image.yml --limit $1
    ssh ${REMOTE_WEB_USER}@$1 ansible-playbook /opt/deploy/devimage-update.yml --limit Sys-Units
    ;;
  "proxyui")
