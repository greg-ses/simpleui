...
@Component({
    ...
    templateUrl: './app-tab-dashboard.component.html',
...
export class AppTabDashboardComponent implements AfterViewInit, OnInit {
    @Input() _uiTab: TabUI;
    @Input() _cmdBarNames: any;
    @Input() _dataTableNames: any;
    @Input() _imageDashboardGroupNames: string;
    @Input() _appURI: string;
    private _nthDashboard = 1;
    private _detectChanges: {name: string, value: boolean};

    _index: number;
    _debug = false;
    _siteIndex = new SiteIndex();
    _implementedDashboards: any;
    _ticks = 0;
    _logCount = 0;
    ...
    ngOnInit() {
        this.getImplementedDashboards(this._nthDashboard);
    }
    ...
    getNthDashboardNumber() {

        let nthDashboard: any;
        nthDashboard = (this._siteIndex && this._siteIndex['props']
            && this._siteIndex['props'].nthDashboard
            && (this._siteIndex['props'].nthDashboard.value)) || 1;

        return nthDashboard;
    }

    getImplementedDashboards(nthDashboard) {

        ClientLogger.log('LogDashboardList', 'getImplementedDashboards(nthDashboard)');

        const ajaxRequest = {
            url: `${this._appURI}php/css_elements_to_json.php?nthDashboard=${nthDashboard}`,
        ...
        data$.subscribe(res => {
            if (   (res.status === 200)
                ...
                return (this.onImplementedDashboardsUpdate(res.response.CSS_Elements));
            } else {
                console.log(`Error ${res.status} fetching res,response.CSS_Elements data in getImplementedDashboards(${nthDashboard})`);
            }
        },
        err => {
            console.log(`Error in AppTabDashboardComponent.getImplementedDashboards() ajax subscribe callback.`);
            try {
                console.log('  name: ' + err.name + ', message: ' + err.message + ', url: ' + err.request.url);
            } catch (err1) { }
        });

        ClientLogger.log('LogDashboardList',
            (typeof this._implementedDashboards === 'undefined') ? 'undefined' : this._implementedDashboards.toString());
    }

    onImplementedDashboardsUpdate(cssElements: any) {
        this._implementedDashboards = [];
        if (cssElements instanceof Object) {
            for (const key of Object.keys(cssElements)) {
                this._implementedDashboards[key] = cssElements[key];
            }
        }
        ClientLogger.log('LogDashboardListAll',
            'onImplementedDashboardsUpdate() - count _implementedDashboards: '
                  + Object.keys(this._implementedDashboards).length);
    }
    ...
    sendCssUpdate(cssOperation: string, cssRecord: string, cssValueRecord: string) {
        ...
        this._cssUpdateService.sendUpdate(this.getNthDashboardNumber(), cssOperation, cssRecord, cssValueRecord);
    }
    ...
    onDataUpdate(response: any) {
        if (typeof response['Dashboard_Summary'] === 'object') {
            this._uiTab._DataSummary = response['Dashboard_Summary'];
        } else {
            this._uiTab._DataSummary = response;
            }
        this.normalizeSection('StatusOverview', 'Status Overview');

        const groups = this._imageDashboardGroupNames.split(',');
    ...
    getDashboardImage() {
        return this._uiTab['dashboardImageUrl'];
    }
    ...
    onDrop(ev: DragEvent) {
        try {
            ...
            if (id === 'unimplementedDashboardsContainer') {
                return false;
            }

            console.log('DashboardComponent.onDrop(' + id + ')');
    ...
    moveDraggableVarOrImage(ev: any, imgData: any, id: string) {
        ...
        const anchorElement = document.getElementById('dashboardAnchor');
        if (anchorElement) {
        ...
        if (imgData) {
            ...
            if (this._debug) {
                const imgInfoCmd = AppTabDashboardComponent.getImgInfoCmd(id, 'Before');
            ...
            if (this._debug) {
                const imgInfoCmd = AppTabDashboardComponent.getImgInfoCmd(id, 'After');
            ...
            const cmd =
                ...
                if (e) {
                     var dashboardAnchor = document.getElementById("dashboardAnchor");
                     if (dashboardAnchor) {dashboardAnchor.appendChild(e);}
                     e.style="${newStyle}";
                 }
