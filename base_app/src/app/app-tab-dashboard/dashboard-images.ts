...
@Component({
    selector: 'image-dashboards',
    changeDetection: ChangeDetectionStrategy.OnPush,
    styleUrls: ['./image-dashboards.css'],
    templateUrl: './image-dashboards.html',
    providers: [CommandButtonChangeService]
})

export class ImageDashboardsComponent {
    @Input() _uiObjList: UiObjList;
    @Input() _dashboardImage: string;
    @Input() _groupName: string;
    @Input() _implementedDashboards: any;
    ...
    formatValue(varName: string, isImplemented: boolean) {
        ...
        try {
            ...
            if (isImplemented) {
                css = this.getImpDashboardCssDef(varName);
                if (css.length > 0) {
    ...
    getImplDashboardName(eName: string): string {
        let retVal = '';
        if (this._implementedDashboards && this._implementedDashboards[eName]) {
            retVal = eName;
        }
        return retVal;
    }
    ...
    getImpDashboardCssDef(eName: string): string {
        let retVal = '';
        if (this._implementedDashboards && this._implementedDashboards[eName]) {
            retVal = this._implementedDashboards[eName].replace(/"/g, '\'');
        }
        return retVal;
    }
    ...
    getImplDashboardImageCssDef(eName: string, eValue: string): string {
        ...
        if (this._implementedDashboards && this._implementedDashboards[cssKey]) {
            retVal = this._implementedDashboards[cssKey];
        } else if (this._implementedDashboards && this._implementedDashboards[eName]) {
            retVal = this._implementedDashboards[eName];
        }
    ...
    getImgSrc(varName: string): string {
        ...
        if (Object.keys(this._uiObjList).indexOf(nameWithoutGroup) !== -1) {
            ...
            if (Object.keys(this._uiObjList).indexOf(nameWithoutGroup) !== -1) {
                ...
                css = this.getImplDashboardImageCssDef(nameWithoutGroup, this._uiObjList[nameWithoutGroup].value);
            } else {
        ...
        let s = this._uiTab
            && this._uiTab['dashboardImageUrl']
            && this._uiTab['dashboardImageUrl'].substring(0, this._uiTab['dashboardImageUrl'].lastIndexOf('/'))
        ...
        css = this.getImplDashboardImageCssDef(nameWithoutGroup, fnValue);
        if (css.length > 0) {
    ...
    onDragStart(ev: any): any {
        if (this._logLevel) {
            const msg = ImageDashboardsComponent.evtToString(ev);
