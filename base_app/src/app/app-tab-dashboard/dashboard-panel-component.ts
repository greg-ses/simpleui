...
@Component({
    selector: 'dashboard-ui-panel',
    styleUrls: [],
    templateUrl: './dashboard-ui-panel-components.html',
    providers: [
        HttpClientModule
    ]
})
...
class DashboardPanelComponent {
    constructor() {
        setTimeout(() => DashboardPanelComponent.registerEventHandlers(), 10);
    }
    ...
    static getSectionNames() {
        ...
        const section = '';
        const sections = [];
        const controlList = DashboardPanelComponent.getControlList();
    ...
    create() {
        ...
        const sections = DashboardPanelComponent.getSectionNames();
...
<select id='sections' onchange='DashboardPanelComponent.onViewSection(event)'>
${sectionOptions}
</select>
</div>
...
`;
        ...
        DashboardPanelComponent.viewSection(sections[0]);
    } // create()

    static onViewSection(event) {
        const section = event.target.options[event.target.selectedIndex].value;
        setTimeout((newSection = section) => DashboardPanelComponent.viewSection(newSection), 1);
    }
...
<th>Action</th>
        ...
        const controls = DashboardPanelComponent.getControlList();
        let i = 0;
        for (const control of controls) {
