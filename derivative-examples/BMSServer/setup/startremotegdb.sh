...
function cmdtarget {
  sshpass -p "Br0myne" ssh service@$targetIp 'echo Br0myne | sudo -S' "$@"
}
...
sshpass -p "Br0myne" scp ../../../../output/$binName/bin/debug/$binName service@$targetIp:.
...
#    basePropName=$(basename $3)
    sshpass -p "Br0myne" scp $3 service@$targetIp:/opt/config/.
    filename=$(basename "$3")
fi
